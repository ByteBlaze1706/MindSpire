...
export default async function DashboardLayout({
  ...
  // Fetch anonymous profile details if they exist
  ...
  const navLinks = [
    ...
    { href: "/playhub", label: 'PlayHub Tools' },
    { href: "/notifications", label: 'Notification Center' },
    { href: "/settings", label: 'Settings' },
  ];

  if (isStaff) {
    navLinks.push({ href: "/moderation", label: 'Moderation Queue' });
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-[#FDFBF7] via-[#F5EBE6] to-[#E3EFF3] flex flex-col md:flex-row antialiased">
      ...
      <aside className="hidden md:flex w-64 bg-white/70 backdrop-blur-md border-r border-neutral-100 flex-col p-6 space-y-8 sticky top-0 h-screen">
        ...
        <div className="border-t border-neutral-100 pt-6">
          ...
          <span className="block text-xs font-semibold text-neutral-700 truncate mt-1">
            {anonProfile?.pseudonym || profile.email}
          </span>
          {anonProfile?.token_id && (
            <span className="block text-[10px] font-mono text-neutral-500 mt-0.5">
              Token: {anonProfile.token_id}
            </span>
          )}
      ...
      <MobileNav navLinks={navLinks} profileEmail={anonProfile?.pseudonym || profile.email} profileRole={profile.role} />
