...
public class QueryUsersTriggers {
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password);
            ...
            System.out.println("Querying custom triggers on public.users...");
            String query = "SELECT tgname AS trigger_name, proname AS function_name " +
                           "FROM pg_trigger " +
                           "JOIN pg_class ON pg_class.oid = tgrelid " +
                           "JOIN pg_proc ON pg_proc.oid = tgfoid " +
                           "WHERE relname = 'users' AND tgisinternal = false;";
            
            try (ResultSet rs = stmt.executeQuery(query)) {
                while (rs.next()) {
                    System.out.printf("Trigger: %-35s | Function: %s%n",
